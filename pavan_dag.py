...
class TeamBeliefDAG:
    ...
    def __init__(self, game: "Game", team_players: Iterable[int]):
        ...
        self.layer_node_to_infoset = defaultdict(set)
        self.infoset_to_layer_nodes = defaultdict(set)
    ...
    def load_conn_graph(self) -> None:
        ...
        with open(ckpt_dir / f"progress_depth_22.pkl", "rb") as f:
            ...
            if "layer_node_to_infoset" in state:
                loaded = defaultdict(set)
                for k, v in state["layer_node_to_infoset"].items():
                    if isinstance(v, (set, frozenset)):
                        loaded[k] = set(v)
                    else:
                        loaded[k].add(v)
                self.layer_node_to_infoset = loaded
            else:
                self.layer_node_to_infoset = defaultdict(set, state["layer_node_to_infoset"])

            itln = defaultdict(set)
            for k, v in state["infoset_to_layer_nodes"].items():
                if isinstance(v, (set, frozenset)):
                    itln[k] = set(v)
                else:
                    itln[k] = set(v)
            self.infoset_to_layer_nodes = itln
    ...
    def _build_connectivity_graph(self) -> None:
        ...
        for d, nodes_d in nodes_by_depth.items():
            if d <= last_depth:
                continue
            for node in nodes_d:
                for infoset_name in self._anc_infosets.get(node, set()):
                    self.layer_node_to_infoset[node].add(infoset_name)
                    self.infoset_to_layer_nodes[(infoset_name, d)].add(node)

            #Pickle at each depth for retrieval later
            checkpoint = {
                "depth": d,
                "layer_node_to_infoset": dict(self.layer_node_to_infoset),
                "infoset_to_layer_nodes": dict(self.infoset_to_layer_nodes),
            }
    ...
    def _connected_components_subset(self, subset: ObsLabel) -> List[Set[str]]:
        ...
        for start in O:
            ...
            infos = self.layer_node_to_infoset.get(start, set())
            if not infos:
                comps.append({start})
                seen.add(start)
                continue
            ...
            while stack:
                ...
                infos_v = self.layer_node_to_infoset.get(v, set())
                if not infos_v:
                    continue

                dv = self.depth.get(v, 0)
                for info_name in infos_v:
                    for nbr in self.infoset_to_layer_nodes.get((info_name, dv), set()):
                        if nbr != v and nbr not in seen and nbr in O:
                            seen.add(nbr)
                            stack.append(nbr)
